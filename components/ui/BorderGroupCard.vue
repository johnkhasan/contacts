  ...
  >
    <div class="flex p-4 justify-between bg-surface-30 rounded-t-lg">
      <span class="font-semibold"
        >{{ $t("scales.sostavNumber") }}: {{ item.sostavNumber }}</span
      >
    </div>

    <div class="flex p-4 flex-col gap-1">
      <div class="flex justify-between items-center">
        <span class="text-sm font-semibold">{{ $t("station.clean") }}</span>
        <span class="text-sm font-bold text-green-500">{{
          Number(item.totalCount) - Number(item.extraCount)
        }}</span>
      </div>

      <div class="flex justify-between items-center">
        <span class="text-sm font-semibold">{{ $t("station.overload") }}</span>
...
type CountBySostavResponseItem =
  components["schemas"]["CountBySostavResponseItem"];

defineProps({
  item: {
    type: Object as () => CountBySostavResponseItem,
    required: true
  }
});
</script>
