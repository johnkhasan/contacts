<template>
  <Modal v-model="openExportModel" :title="$t('wagon.dowloadAsExcel')">
    <ScaleCustomsExport @cancel="onCancel" />
  </Modal>

  <div class="flex-1 flex flex-col gap-5 overflow-auto" v-loading="status === 'pending'">
    <ScaleCustomsFilter v-model="filterParams" @export="onExport" @get="execute" />

    <ScaleCustomsList v-model="filterParams" :data="data!" @get="execute" />
  </div>
</template>
